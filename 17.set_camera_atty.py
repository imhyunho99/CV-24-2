...
def put_string(frame, text, pt, value=None, color=(120, 200, 90)) :
    ...
    cv.putText(frame, text, pt   , font, 0.7, color, 2) # 작성 문자
    
def zoom_bar(value):
    global capture
    capture.set(cv.CAP_PROP_ZOOM, value) # 줌 설정 
...
capture.set(cv.CAP_PROP_FRAME_HEIGHT, 300)     # 카메라 프레임 높이
capture.set(cv.CAP_PROP_AUTOFOCUS, 0)          # 오토포커싱 중지
capture.set(cv.CAP_PROP_BRIGHTNESS, 100)       # 프레임 밝기 초기화

title = "Change Camera Properties"              # 윈도우 이름 지정
cv.namedWindow(title)                          # 윈도우 생성 - 반드시 생성 해야함
cv.createTrackbar("zoom" , title, 0, 10, zoom_bar)
cv.createTrackbar("focus", title, 0, 40, focus_bar)
